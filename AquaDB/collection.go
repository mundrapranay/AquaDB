...
func (c *Collection) Remove(key []byte) error {
	...
	removeItemIndex, nodeToRemoveFrom, ancestorsIndex, err := rootNode.findKey(key, true)
	if err != nil {
		return err
	}
	...
	if nodeToRemoveFrom.isLeaf() {
		...
		ancestorsIndex = append(ancestorsIndex, affectedNodes...)
	}

	ancestors, err := c.getNodes(ancestorsIndex)
	if err != nil {
		return err
	}
	...
	for i := len(ancestors) - 2; i >= 0; i-- {
		...
		if node.isUnderPopulated() {
			err = pnode.rebalanceRemove(node, ancestorsIndex[i+1])
			if err != nil {
				return err
			}
		}
	...
	rootNode = ancestors[0]
	// if root has no item after rebalancing, there's no need to save it
