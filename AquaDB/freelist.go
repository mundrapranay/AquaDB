...
// metaPage is the maximum pgnum that is used by the db for its own purposes. (header page)
const metaPage = 0

type freelist struct {
	maxPage       pgnum   // maximum pages allocated
	releasedPages []pgnum // pages that were previously allocated but are now free
}
