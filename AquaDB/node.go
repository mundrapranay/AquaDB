...
// The merge function receives a node and its index. Then, it merges that node with its left sibling,
// transferring its key-value pairs and child pointers. After the function finishes, it deletes the node
...
func rotateRight(aNode, pNode, bNode *Node, bNodeIndex int) {
	...
	// Get last time and remove it
	aNodeItem := aNode.items[len(aNode.items)-1]
	aNode.items = aNode.items[:len(aNode.items)-1]

	// get item from parent node and assign the aNodeItem item
...
func rotateLeft(aNode, pNode, bNode *Node, bNodeIndex int) {
	...
	// Get first item and remvoe it
	bNodeItem := bNode.items[0]
	bNode.items = bNode.items[1:]

	// Get item from parent node and assign the bNodeItem item
...
func (n *Node) removeItemFromInternal(index int) ([]int, error) {
	...
	// starting from its left child descend to the rightmost desendant
	...
	// replace the item that should be removed with the
	// item before inorder which we just saw
...
// removeItemFromLeaf removes an item from a leaf node
